...
'common.copy': '复制',
    ...
    'pagination.pageLabel': '页',
            'home.tagline': 'AI 开发工具统一接入平台，整合多个 AI 编程助手',
    'home.openConsole': '控制台',
    'home.enterConsole': '进入控制台',
    'home.getStart': '开始使用',
    'home.viewStatus': '服务状态',
    'home.coreFeatures': '核心能力',
    'home.coreFeaturesSub': '从接入、路由到计费，一站式管理你的 AI 网关',
    'home.ctaTitle': '准备好开始了吗？',
    'home.ctaSub': '创建账号并生成 API Key，几分钟内接入你的第一个 AI 编程助手。',
'palette.title': '命令面板',
    'palette.placeholder': '搜索页面与功能…',
    'palette.empty': '没有匹配的结果',
    'palette.hint': '输入以搜索页面',
...
'common.copy': 'Copy',
    ...
    'pagination.pageLabel': 'page',
            'home.tagline': 'One gateway for all your AI coding assistants',
    'home.openConsole': 'Console',
    'home.enterConsole': 'Open console',
    'home.getStart': 'Get started',
    'home.viewStatus': 'Service status',
    'home.coreFeatures': 'Core capabilities',
    'home.coreFeaturesSub': 'Access, routing, and billing — manage your AI gateway in one place',
    'home.ctaTitle': 'Ready to get started?',
    'home.ctaSub': 'Create an account, generate an API key, and connect your first AI coding assistant in minutes.',
'palette.title': 'Command palette',
    'palette.placeholder': 'Search pages and features…',
    'palette.empty': 'No matching results',
    'palette.hint': 'Type to search pages',
'pagination.pageNumber': 'Page {page}',
    ...
    'nav.vscodex': 'Remote control',
    'vscodex.title': 'Remote control',
    ...
    'vscodex.frame.title': 'Remote control surface',
    ...
    'models.management.batch.description': 'Prices sync from each model’s source saved in the database by default, or you can choose one source for all selected models.',
    ...
    'models.management.batch.rememberedProviderLabel': 'each model’s saved provider',
...
const legacyExactEnglishMessages: Record<string, string> = {
  ...
  '配额暂未就绪，请稍后刷新': 'Quota information is not ready yet. Please refresh again shortly.',
...
const legacyDynamicPatterns: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
  ...
  [/^(\d+)天([01]?\d|2[0-3])时$/u, match => `${match[1]}d ${match[2]}h`],
  [/^(\d+)天\s+([01]?\d|2[0-3]):([0-5]\d):([0-5]\d)$/u, match => `${match[1]}d ${match[2]}:${match[3]}:${match[4]}`],
  [/^成功率 (\d[\d.,]*%)$/u, match => `Success rate ${match[1]}`],
  [/^输入 ([\d.,]+[KMB]?) \/ 输出 ([\d.,]+[KMB]?)$/u, match => `Input ${match[1]} / Output ${match[2]}`],
  [/^节省 (\$[\d.,]+) \((\d[\d.,]*%)\)$/u, match => `Saved ${match[1]} (${match[2]})`],
  [/^总用户 (\d[\d,]*)$/u, match => `Total users ${match[1]}`],
  ...
  [/^(启用|禁用|删除|创建|更新)用户失败$/u, match => `Failed to ${translateLegacyText(match[1], 'en-US').toLowerCase()} user`],
  ...
  [/^(.+)包含无效的 URL 编码$/u, match => `${translateLegacyText(match[1], 'en-US')} contains invalid URL encoding`],
