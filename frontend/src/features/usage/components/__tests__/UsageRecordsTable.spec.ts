...
describe('UsageRecordsTable', () => {
  ...
  it('shows mapping, reasoning, Fast, and Cyber in the model area', () => {
    ...
    expect(fastBadge?.classList.contains('border-[#ffb357]/50')).toBe(false)
    expect(fastBadge?.classList.contains('!bg-transparent')).toBe(false)
    expect(fastBadge?.classList.contains('bg-[#ffb357]/10')).toBe(false)
    expect(fastBadge?.classList.contains('text-[#4080ff]')).toBe(true)
    ...
    expect(cyberBadge?.classList.contains('text-[#f53f3f]')).toBe(true)
