...
const variants = {
  ...
  interactive: 'rounded-lg border border-border bg-card text-card-foreground transition-all duration-200 hover:border-primary/40',

  // 轻量卡片 - 更淡的边框,用于辅助信息区域
  subtle: 'rounded-lg border border-border/60 bg-card text-card-foreground',

  // 嵌入式卡片 - 极淡背景,用于卡片内的子卡片(保持层级关系)
  inset: 'rounded-lg border border-border/50 bg-muted/40 text-card-foreground'
}
