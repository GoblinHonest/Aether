<template>
  <div class="min-h-screen bg-white dark:bg-[#17171a]">
    <!-- Nav -->
    <nav class="sticky top-0 z-50 bg-white/80 dark:bg-[#17171a]/80 backdrop-blur-sm border-b border-[#eee] dark:border-[#222]">
      <div class="max-w-[1200px] mx-auto px-8 h-16 flex items-center justify-between">
        <span class="text-base font-semibold text-[#2a2a2b] dark:text-[#e5e5e5]">{{ siteName || 'ManyCode' }}</span>
        <div class="flex items-center gap-6">
          <button @click="toggleDarkMode" class="w-8 h-8 flex items-center justify-center rounded-lg text-[#999] hover:text-[#2a2a2b] dark:hover:text-[#e5e5e5] transition-colors cursor-pointer">
            <Sun v-if="isDark" class="w-4 h-4" />
            <Moon v-else class="w-4 h-4" />
          </button>
          <router-link v-if="authStore.isAuthenticated" :to="dashboardPath" class="text-sm bg-[#2a2a2b] dark:bg-[#e5e5e5] text-white dark:text-[#17171a] px-5 py-2 rounded-full font-medium hover:opacity-90 transition-opacity cursor-pointer">
            控制台
          </router-link>
          <button v-else @click="showLogin = true" class="text-sm bg-[#2a2a2b] dark:bg-[#e5e5e5] text-white dark:text-[#17171a] px-5 py-2 rounded-full font-medium hover:opacity-90 transition-opacity cursor-pointer">
            登录
          </button>
        </div>
      </div>
    ...
    <section class="py-20 md:py-28">
      <div class="max-w-[1200px] mx-auto px-8 flex flex-col lg:flex-row items-start gap-12 lg:gap-20">
        ...
        <div class="flex-1 pt-4">
          <h1 class="text-4xl md:text-5xl lg:text-6xl font-normal text-[#2a2a2b] dark:text-[#e5e5e5] tracking-tight leading-tight">{{ siteName || 'ManyCode' }}</h1>
          ...
          <div class="mt-8 flex items-center gap-4">
            <router-link v-if="authStore.isAuthenticated" :to="dashboardPath" class="bg-[#2a2a2b] dark:bg-[#e5e5e5] text-white dark:text-[#17171a] px-7 py-3 rounded-full text-sm font-medium hover:opacity-90 transition-opacity cursor-pointer">
              进入控制台
            </router-link>
            <button v-else @click="showLogin = true" class="bg-[#2a2a2b] dark:bg-[#e5e5e5] text-white dark:text-[#17171a] px-7 py-3 rounded-full text-sm font-medium hover:opacity-90 transition-opacity cursor-pointer">
              开始使用
            </button>
          </div>
        </div>
        ...
        <div class="flex-1 w-full lg:max-w-[520px]">
          ...
          <div class="flex gap-1 mb-4">
            <button
              ...
              :class="activeTab === tab.key
                ? 'bg-[#2a2a2b] dark:bg-[#e5e5e5] text-white dark:text-[#17171a]'
                : 'text-[#666] dark:text-[#999] hover:bg-[#f5f5f5] dark:hover:bg-[#2a2a2b]'"
            >
              {{ tab.label }}
            </button>
          </div>
          ...
          <!-- Code block -->
          <div class="bg-[#17171a] rounded-2xl p-5 relative">
      ...
      <div class="max-w-[1200px] mx-auto px-8">
        <div class="grid md:grid-cols-3 gap-5">
          <div class="bg-[#f5f5f5] dark:bg-[#111] rounded-2xl p-8">
            <p class="text-4xl font-normal text-[#2a2a2b] dark:text-[#e5e5e5]">99.9%</p>
          ...
          <div class="bg-[#f5f5f5] dark:bg-[#111] rounded-2xl p-8">
            <p class="text-4xl font-normal text-[#2a2a2b] dark:text-[#e5e5e5]">~180ms</p>
            <p class="mt-2 text-sm text-[#999]">平均响应延迟</p>
            <p class="mt-4 text-[13px] text-[#666] dark:text-[#999] leading-relaxed">智能路由选择最优链路，首字节极速返回</p>
          </div>
          <div class="bg-[#f5f5f5] dark:bg-[#111] rounded-2xl p-8">
            <p class="text-4xl font-normal text-[#2a2a2b] dark:text-[#e5e5e5]">42+</p>
    ...
    <section class="py-24">
      <div class="max-w-[1200px] mx-auto px-8">
        <h2 class="text-3xl font-semibold text-[#2a2a2b] dark:text-[#e5e5e5] mb-12">核心能力</h2>
        <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          <div v-for="(feature, i) in features" :key="i" class="bg-[#f5f5f5] dark:bg-[#111] rounded-2xl p-7">
            <h3 class="text-base font-semibold text-[#2a2a2b] dark:text-[#e5e5e5] mb-2">{{ feature.title }}</h3>
